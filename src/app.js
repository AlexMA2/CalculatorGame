...
const correctIcon =
  '<svg id="correct" height="30" viewBox="0 0 21 21" width="30" xmlns="http://www.w3.org/2000/svg"><path d="m.5 5.5 3 3 8.028-8" fill="none" stroke="green" stroke-linecap="round" stroke-linejoin="round" transform="translate(5 6)"/></svg>';
const incorrectIcon =
  '<svg id="incorrect" height="30" viewBox="0 0 21 21" width="30" xmlns="http://www.w3.org/2000/svg"><g fill="none" fill-rule="evenodd" stroke="red" stroke-linecap="round" stroke-linejoin="round" transform="translate(5 5)"><path d="m10.5 10.5-10-10z"/><path d="m10.5.5-10 10"/></g></svg>';

const createTableDetails = () => {
  let tbody = "";
  for (let i = 0; i < numQuestion - 1; i++) {
    if (resultsOfAnswers[i] === 0) {
      tbody += createRowInTable(
        i + 1,
        questions[i].slice(0, -2),
        parseInt(answers[i]),
        correctAnswers[i],
        incorrectIcon
      );
